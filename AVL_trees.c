...
void rebalance(NODE** root, NODE* node){
    ...
    adjustHeight(node);
    if(node->parent != NULL) rebalance(root, node->parent);
}
...
void rebalanceRight(NODE** root, NODE* node){
    if(getBalance(node->left) < 0) rotateLeft(root, node->left);
...
void rebalanceLeft(NODE** root, NODE* node){
    if(getBalance(node->right) > 0) rotateRight(root, node->right);
...
void BFS(NODE* root){
    ...
    while(queue->head != NULL){
        ...
        printf("levelSize = %d -> ", currentSize);
